...
export const ordersApi = {
  ...
  getActive: async (): Promise<Order[]> => {
    const response = await apiClient.get<ApiResponse<Order[]>>('/admin/orders/active');
  ...
  cancel: async (id: string): Promise<Order> => {
    const response = await apiClient.delete<ApiResponse<Order>>(`/admin/orders/${id}`);
    return response.data.data;
  },
  ...
  getByTable: async (tableId: string): Promise<Order[]> => {
    const response = await apiClient.get<ApiResponse<Order[]>>(`/admin/orders/table/${tableId}`);
    return response.data.data;
  },
