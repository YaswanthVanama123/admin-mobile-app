...
export const kitchenApi = {
   ...
   */
  getPreparationQueue: async (): Promise<Order[]> => {
    const response = await apiClient.get<ApiResponse<Order[]>>('/kitchen/queue');
    return response.data.data;
  },
   ...
   */
  updatePriority: async (orderId: string, priority: 'high' | 'normal' | 'low'): Promise<Order> => {
    const response = await apiClient.patch<ApiResponse<Order>>(
