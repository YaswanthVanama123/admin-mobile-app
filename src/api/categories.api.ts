...
export const categoriesApi = {
   ...
   * Reorder categories
   */
  reorder: async (categories: Array<{ id: string; displayOrder: number }>): Promise<void> => {
   ...
   */
  getWithItemCount: async (): Promise<Array<Category & { itemCount: number }>> => {
    const response = await apiClient.get<ApiResponse<Array<Category & { itemCount: number }>>>(
