...
import { AddOn, AddOnFormData, ApiResponse } from './types';
...
export const addOnsApi = {
  ...
  toggleAvailability: async (id: string): Promise<AddOn> => {
    const response = await apiClient.patch<ApiResponse<AddOn>>(`/addons/${id}/toggle-availability`);
    return response.data.data;
  },
   ...
   * Reorder add-ons
   */
  reorder: async (addOns: Array<{ id: string; displayOrder: number }>): Promise<void> => {
    await apiClient.post('/addons/reorder', { addOns });
  },
